...
static int prep_next_send(struct io_uring *ring,
                          struct conn *c,
                          struct conn_dir *cd,
                          int fd);
...
{
    const char *name;
    int (*error_fn)(struct error_handler *,
                    struct io_uring *,
                    struct io_uring_cqe *);
};

static int recv_error(struct error_handler *err,
                      struct io_uring *ring,
                      struct io_uring_cqe *cqe);
static int send_error(struct error_handler *err,
                      struct io_uring *ring,
                      struct io_uring_cqe *cqe);
...
static int setup_recv_ring(struct io_uring *ring, struct conn *c)
{
    ...
    printf(
        "%d: recv buffer ring bgid %d, bufs %d\n", c->tid, cbr->bgid, nr_bufs);
    return 0;
}
...
static int setup_send_ring(struct io_uring *ring, struct conn *c)
{
    ...
    printf(
        "%d: send buffer ring bgid %d, bufs %d\n", c->tid, cbr->bgid, nr_bufs);
    return 0;
}
...
static void __show_stats(struct conn *c)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        printf(
            "\t%3d: rcv=%u (short=%u, enobufs=%d), snd=%u (short=%u,"
            " busy=%u, enobufs=%d)\n",
            i,
            cd->rcv,
            cd->rcv_shrt,
 ...
 */
static void encode_userdata(
    struct io_uring_sqe *sqe, struct conn *c, int op, int bid, int fd)
{
    __encode_userdata(sqe, c->tid, op, bid, fd);
}

static void __submit_receive(struct io_uring *ring,
                             struct conn *c,
                             struct conn_dir *cd,
                             int fd)
{
...
static int handle_accept(struct io_uring *ring, struct io_uring_cqe *cqe)
{
    ...
    else
    {
        ...
        io_uring_prep_msg_ring_fd(
            sqe, c->ring.ring_fd, cqe->res, 0, user_data, 0);
...
static int handle_sock(struct io_uring *ring, struct io_uring_cqe *cqe)
{
    ...
    if (ipv6)
    {
        io_uring_prep_connect(
            sqe, c->out_fd, (struct sockaddr *) &c->addr6, sizeof(c->addr6));
    }
    else
    {
        io_uring_prep_connect(
            sqe, c->out_fd, (struct sockaddr *) &c->addr, sizeof(c->addr));
    }
...
static void send_append_vec(struct conn_dir *cd, void *data, int len)
{
    ...
    if (mvec->iov_len == mvec->vec_size)
    {
        mvec->vec_size <<= 1;
        mvec->iov =
            (iovec *) realloc(mvec->iov, mvec->vec_size * sizeof(struct iovec));
    }
 ...
 */
static void send_append(
    struct conn *c, struct conn_dir *cd, void *data, int bid, int len)
{
 ...
 * outgoing queue.
 */
static int recv_bids(struct conn *c,
                     struct conn_dir *cd,
                     int *bid,
                     int in_bytes)
{
    ...
    while (in_bytes)
    {
        ...
        if (send_ring)
            io_uring_buf_ring_add(
                cbr->br, data, this_bytes, *bid, br_mask, nr_packets);
        else
 ...
 */
static int recv_mshot_msg(struct conn *c,
                          struct conn_dir *cd,
                          int *bid,
                          int in_bytes)
{
...
static int __handle_recv(struct io_uring *ring,
                         struct conn *c,
                         struct conn_dir *cd,
                         struct io_uring_cqe *cqe)
{
...
static void submit_send(struct io_uring *ring,
                        struct conn *c,
                        struct conn_dir *cd,
                        int fd,
                        void *data,
 ...
 */
static int handle_send_ring(struct conn *c,
                            struct conn_dir *cd,
                            int bid,
                            int bytes)
{
 ...
 * ring for receives.
 */
static int handle_send_buf(struct conn *c,
                           struct conn_dir *cd,
                           int bid,
                           int bytes)
{
...
static int __handle_send(struct io_uring *ring,
                         struct conn *c,
                         struct conn_dir *cd,
                         struct io_uring_cqe *cqe)
{
...
static int init_ring(struct io_uring *ring, int nr_files)
{
    ...
    else
    {
        ...
        {
            fprintf(stderr,
                    "Kernel doesn't support ring provided "
...
int main(int argc, char *argv[])
{
    ...
    printf(
        "Backend: sqpoll=%d, defer_tw=%d, fixed_files=%d, "
    ...
    printf(" recv options: recvmsg=%d, recv_mshot=%d, recv_bundle=%d\n",
           ...
           rcv_bundle);
    printf(
        " send options: sendmsg=%d, send_ring=%d, send_bundle=%d, "
        "send_zerocopy=%d\n",
        snd_msg,
        send_ring,
        snd_bundle,
