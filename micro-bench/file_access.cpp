...
// the expected API for a file access class, read, write, read_many, write_many,
// and fsync
class AsyncFileBase
{
public:
    ...
    virtual std::future<IOResult> read_many(
        vector<pair<size_t, char *>> &pages) = 0;
    virtual std::future<IOResult> write_many(
        vector<pair<size_t, char *>> &pages) = 0;
    ...
    IOResult read_many_posix(vector<pair<size_t, char *>> &pages)
    {
        ...
        {
            size_t read_bytes =
                pread(fd_, page.second, PAGE_SIZE, page.first * PAGE_SIZE);
            assert(read_bytes == PAGE_SIZE && "Failed to read full page");
        }
        return SUCCESS;
    }
    ...
    IOResult write_many_posix(vector<pair<size_t, char *>> &pages)
    {
        for (auto &page : pages)
        {
            size_t written_bytes =
                pwrite(fd_, page.second, PAGE_SIZE, page.first * PAGE_SIZE);
            assert(written_bytes == PAGE_SIZE && "Failed to write full page");
        }
        return SUCCESS;
    }
...
public:
    ...
    // using posix read and write, return a future object that is always
    // fulfilled
    ...
    std::future<IOResult> read_many(
        vector<pair<size_t, char *>> &pages) override
    {
        std::promise<IOResult> promise;
        std::future<IOResult> future = promise.get_future();
    ...
    std::future<IOResult> write_many(
        vector<pair<size_t, char *>> &pages) override
    {
        std::promise<IOResult> promise;
        std::future<IOResult> future = promise.get_future();
...
// using std::async to wrap the sychronous posix read and write, but return a
// future object that is fulfilled asynchronously std::async starts up a thread
// everytime it is called, so it is not fast
class AsyncFileAsync : public AsyncFileBase
{
public:
    AsyncFileAsync(const std::string &filename)
        : AsyncFileBase(filename), outstanding_writes_(0), allow_writes_(true)
    {
    }
    ...
    void fsync_file(bool data_only) override
    {
        ...
        while (!allow_writes_.compare_exchange_weak(expected, false))
            std::this_thread::yield();  // already in fsync, wait for it to
                                        // finish
    ...
    {
        return std::async(std::launch::async,
                          [this, &pages] { return read_many_posix(pages); });
    }
        ...
        vector<pair<size_t, char *>> &pages) override
    {
        ...
        outstanding_writes_++;
        return std::async(std::launch::async,
                          [this, &pages] { return write_many_posix(pages); });
    }
...
{
public:
    IOSlotManager(size_t slot_count = 128)
        : slots_(slot_count), free_slots_(slot_count)
    {
    ...
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if (slot->pages.capacity() > 1024)
        {  // capacity is not thread safe, I think
...
// using a fixed number of threads in a pool to handle IO operations. Ops are
// inserted into a queue first and the threads will pick up the ops from the
// queue
...
public:
    AsyncFileThreadPool(
        ...
        : AsyncFileBase(filename), slot_manager_(128)
    {
        allow_writes_ = true;
        ...
        {
            threads_.emplace_back([this] { thread_fun(); });
        }
    }
    ~AsyncFileThreadPool()
    {
    ...
    // have to wait till all outstanding writes are processed before fsync to
    // maintain semantic
    void fsync_file(bool data_only) override
    {
        bool expected = true;
        while (!allow_writes_.compare_exchange_weak(expected, false))
            std::this_thread::yield();  // already in fsync, wait for the
...
{
public:
    AsyncFileIOUring(const std::string &filename, size_t queue_size = 32)
        : AsyncFileBase(filename), io_slot_manager_(128)
    {
        stop_polling_ = false;
        outstanding_writes_ = 0;
        allow_writes_ = true;
        ...
        if (r != 0)
        {
            std::cerr << "Failed to initialize io_uring: " << strerror(-r)
                      << std::endl;
            exit(1);
        }

        poll_thread_ =
            std::thread(&AsyncFileIOUring::poll_for_completion, this);
    }
    ...
    {
        bool expected = true;
        while (!allow_writes_.compare_exchange_weak(
            expected, false))  // already in fsync, return;
            std::this_thread::yield();
    ...
    {
        IOSlot *io_slot =
            io_slot_manager_.get_slot(IOSlot::READ_PAGE, pg_idx, buff);
        struct io_uring_sqe *sqe = io_uring_get_sqe(&ring_);
    ...
    std::future<IOResult> write_page(size_t pg_idx, char *buff) override
    {
        ...
        IOSlot *io_slot =
            io_slot_manager_.get_slot(IOSlot::WRITE_PAGE, pg_idx, buff);
        struct io_uring_sqe *sqe = io_uring_get_sqe(&ring_);
        ...
        vector<pair<size_t, char *>> &pages) override
    {
        ...
        for (auto &page : pages)
        {
            struct io_uring_sqe *sqe = io_uring_get_sqe(&ring_);
            io_uring_prep_read(
                sqe, fd_, page.second, PAGE_SIZE, page.first * PAGE_SIZE);
            io_uring_sqe_set_data(sqe, io_slot);
            pending_ops_++;
            if (pending_ops_ >= 32)
            {  // too many pending ops
...
private:
    ...
    {
        std::chrono::time_point<std::chrono::high_resolution_clock> idle_start =
        ...
        while (!stop_polling_)
        {
            ...
            {
                std::this_thread::yield();
                if (std::chrono::high_resolution_clock::now() - idle_start >
                    std::chrono::microseconds(1000 * 10))
                {
                    std::this_thread::sleep_for(
                        std::chrono::microseconds(1000 * 20));
...
{
public:
    AsyncFileIOUringOptimized(const std::string &filename,
        ...
        : AsyncFileBase(filename), io_slot_manager_(128)
    {
        ...
        if (r != 0)
        {
            std::cerr << "Failed to register file to io_uring: " << strerror(-r)
                      << std::endl;
            exit(1);
        }
        poll_thread_ =
            std::thread(&AsyncFileIOUringOptimized::poll_for_completion, this);
    }
    ...
    std::future<IOResult> read_page(size_t pg_idx, char *buff) override
    {
        ...
        io_uring_prep_read_fixed(
            sqe, fd_, buff, PAGE_SIZE, pg_idx * PAGE_SIZE, 0);
        submit_io_op(sqe, io_slot);
        return io_slot->promise.get_future();
    }
        ...
        vector<pair<size_t, char *>> &pages) override
    {
        ...
        for (auto &page : pages)
        {
            struct io_uring_sqe *sqe = io_uring_get_sqe(&ring_);
            io_uring_prep_read_fixed(
                sqe, fd_, page.second, PAGE_SIZE, page.first * PAGE_SIZE, 0);
            io_uring_sqe_set_data(sqe, io_slot);
            pending_ops_++;
            if (pending_ops_ >= 32)
            {  // too many pending ops
             ...
             double read_fraction)
{
    ...
    for (size_t page_id = 0; page_id < num_pages; ++page_id)
    {
        ...
        ssize_t written =
            pwrite(fd, page_content.data(), PAGE_SIZE, page_id * PAGE_SIZE);
        assert(written == PAGE_SIZE && "Failed to write full page");
    }
    close(fd);
    std::cout << "File filled with integers up to page count: " << num_pages
              << std::endl;
    ...
    while (num_issued_ops < num_ops)
    {
        ...
        while (num_issued_ops < num_ops &&
               num_outstanding < max_num_outstanding)
        {
        ...
        for (auto &io_task : io_tasks)
        {
            if (io_task.processed)
                continue;
            if (io_task.future_obj.wait_for(std::chrono::seconds(0)) ==
    ...
    std::cout << "Total Pages " << num_pages << " Total IOs " << num_ops
              << " IOPS" << std::endl;
    std::cout << "Throughput " << num_ops / total_time.count() << std::endl;
    std::cout << "Average IO Latency "
              << total_io_time / num_ops * 1000.0 * 1000.0
              << "us, max IO Latency " << max_io_time * 1000.0 * 1000.0 << "us"
              << std::endl;
};
